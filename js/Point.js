define([], function () {
        ...
        exported.prototype.updateMyPosition = function (camera) {
            ...
            divPoint.style.top = String(window.innerHeight/2 + (image.absolutePos.y + (image.absolutePos.h * 0.5 * this.point.y) + camera.position.y - window.innerHeight/2)*camera.scale) + "px";
            divPoint.style.left =String(window.innerWidth/2 + (image.absolutePos.x + (image.absolutePos.w * 0.5 * this.point.x) + camera.position.x - window.innerWidth/2)*camera.scale) + 'px';
